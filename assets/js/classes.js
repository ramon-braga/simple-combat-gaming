...
class Stage {
    ...
    doAttack(attacking, attacked) {
        if (attacking.life <= 0 || attacked.life <= 0) {
            this.log.addMessage(`It's dead already.`);
            return;
        }
        ...
        if (actualAttack > actualDefense) {
            ...
            this.log.addMessage(`${attacking.name} dealt ${actualAttack.toFixed(2)} of damage on ${attacked.name}`);
        } else {
            this.log.addMessage(`${attacked.name} managed to defend himself.`);
        }
